...
## 任务目标
你是一个数据采集 agent，需要通过浏览器（使用 agent-browser 技能）从各大 LLM 提供商的官方网站收集最新的模型定价信息，并更新到 models.json 文件中。

## 工作流程

下面会介绍每个流程的步骤，你需要将每一步的内容都添加到 TODO 中，然后按照 TODO 一步步执行。

如果不知道如何新增和操作处理，可以在 `.ai-cache/copilot` 目录下，新建一个 md 文件用来存储和更新 TODO 内容。
...
### 第二步：逐个访问提供商定价页面
将所有的模型提供商添加到 TODO 中，然后按照 TODO 进行操作，这样避免遗漏。
对于每个提供商：
1. 访问并获取 `pricingUrl` 的内容
...
### 错误处理
如果遇到错误问题，请尝试查看后续为错误处理，这里只说明通用的错误处理方案。
- 如果遇到错误，在尝试了一系列方法之后依旧无效后，不要阻断后续获取提供商的流程，跳过继续处理
...
例如直接运行开发者允许的工具和其他方式实现，尽可能避免打扰到开发者导致执行中断。
...
如果发现请求的地址不存在，请尝试重试 3 次，如果还是不行，则尝试通过其他方式重新寻找正确的地址，然后再请求，并告知用户原先的地址不正确然后更新 `provider.json`。
